...
### prompt-tuner
Reviews and improves LLM prompt templates. Analyzes clarity, specificity, and potential failure modes. Use during development of `pr_review/prompts/templates.py`.

### test-runner
Runs the test suite and the full analysis pipeline:
```bash
python -m pytest tests/ -v
python analyze_pr.py --pr data/input/pr_input.json --output data/output/code_review_output.json
```
...
### reviewer
Acts as a principal developer reviewer before every commit. Reviews staged changes against:
- **CLAUDE.md** — code style, error handling, architecture alignment, quality standards
- **assessment/ASSIGNMENT.md** — LangGraph usage, output format, CLI interface, technical requirements, what NOT to build
- **assessment/TEST_DATA_INSTRUCTIONS.md** — input/output format compliance, PR analysis correctness

Flags violations, suggests fixes, and blocks commit until all standards are met.

### documentator
Generates/updates README.md with:
- Clear usage instructions (install, configure, run)
- Architecture description with diagram
- Design decisions and reasoning behind them
- Workflow explanation
- Trade-offs considered (accuracy vs. speed, simplicity vs. features)
